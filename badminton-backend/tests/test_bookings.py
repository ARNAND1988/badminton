...
def test_completed_bookings_require_login_and_include_past_confirmed_bookings(client, app):
    ...
    assert data['bookings'][0]['booking_date'] == '2026-06-28'

    all_resp = client.get('/api/bookings?status=completed&page=1&per_page=100', headers={'Authorization': f'Bearer {token}'})
    all_dates = {booking['booking_date'] for booking in all_resp.get_json()['bookings']}
    assert '2020-01-01' in all_dates
    assert '2026-06-28' in all_dates
